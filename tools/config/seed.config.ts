...
export class SeedConfig {
  ...
  */
  ...
  /**
   * The folder for built files in the `mock` environment.
   * @type {string}
   */
  MOCK_DEST = `${this.DIST_DIR}/mock`;

  /**
   * The folder for built files in the `fnta` environment.
   * @type {string}
   */
  FNTA_DEST = `${this.DIST_DIR}/fnta`;

  /**
   * The folder for built files in the `fnte` environment.
   * @type {string}
   */
  FNTE_DEST = `${this.DIST_DIR}/fnte`;

  /**
   * The folder for built files in the `nfta` environment.
   * @type {string}
   */
  NFTA_DEST = `${this.DIST_DIR}/nfta`;
   ...
   */
   ...
   APP_DEST = this.BUILD_TYPE === BUILD_TYPES.DEVELOPMENT ? this.DEV_DEST :
              this.BUILD_TYPE === BUILD_TYPES.MOCK ? this.MOCK_DEST :
              this.BUILD_TYPE === BUILD_TYPES.FNTA ? this.FNTA_DEST :
              this.BUILD_TYPE === BUILD_TYPES.FNTE ? this.FNTE_DEST :
              this.BUILD_TYPE === BUILD_TYPES.NFTA ? this.NFTA_DEST :
              this.PROD_DEST;
   ...
   * @type {string[]}
   */
  ROLLUP_INCLUDE_DIR: string[] = ['node_modules/rxjs/**', 'node_modules/z-schema/dist/**', 'node_modules/commander/**', 'node_modules/lodash.get/**', 'node_modules/lodash.isequal/**', 'node_modules/validator/**'];
  ...
  */
  ...
  SYSTEM_CONFIG_DEV: any = {
    ...
    paths: {
      ...
      'z-schema': 'node_modules/z-schema/dist/ZSchema-browser-min.js',
      'body-parser': 'node_modules/body-parser/index.js',
      'methods': 'node_modules/methods/index.js',
      'raw-loader': 'node_modules/raw-loader/index.js',
  ...
  SYSTEM_BUILDER_CONFIG: any = {
    ...
    packages: {
      ...
      'z-schema': {
        main: 'dist/ZSchema-browser-min.js',
        defaultExtension: 'js'
      }
    }
  };
  ...
  */
  ...
  getInjectableStyleExtension() {
    return this.BUILD_TYPE === BUILD_TYPES.DEVELOPMENT || !this.ENABLE_SCSS ? 'css' : 'scss';
  }
...
function getBuildType() {
  ...
  let mockKeyword = !!base.filter(o => o.indexOf(BUILD_TYPES.MOCK) >= 0).pop();
  let fntaKeyword = !!base.filter(o => o.indexOf(BUILD_TYPES.FNTA) >= 0).pop();
  let fnteKeyword = !!base.filter(o => o.indexOf(BUILD_TYPES.FNTE) >= 0).pop();
  let nftaKeyword = !!base.filter(o => o.indexOf(BUILD_TYPES.NFTA) >= 0).pop();
  let prodKeyword = !!base.filter(o => o.indexOf(BUILD_TYPES.PROD) >= 0).pop();
  if ((base && mockKeyword) || type === BUILD_TYPES.MOCK) {
    return BUILD_TYPES.MOCK;
  } else if ((base && fntaKeyword) || type === BUILD_TYPES.FNTA) {
    return BUILD_TYPES.FNTA;
  } else if ((base && fnteKeyword) || type === BUILD_TYPES.FNTE) {
    return BUILD_TYPES.FNTE;
  } else if ((base && nftaKeyword) || type === BUILD_TYPES.NFTA) {
    return BUILD_TYPES.NFTA;
  } else if ((base && prodKeyword) || type === BUILD_TYPES.PROD) {
    return BUILD_TYPES.PROD;
  } else {
